...
public:
    ...
    class OutDFSVisitor : public default_dfs_visitor {
        std::ostream & os_;
    public:
        OutDFSVisitor(std::ostream & os) : os_(os) { /* None */ }

        void tree_edge(edge_type e, const Graph & g) const {
            os_ << "      tree     " << e << std::endl;
        }

        void back_edge(edge_type e, const Graph & g) const {
            os_ << "non tree black " << e << std::endl;
        }

        void forward_or_cross_edge(edge_type e, const Graph & g) const {
            os_ << "non tree cross " << e << std::endl;
        }
    };
